 ...
 */
/** Calcula previsão de parto a partir de uma data base + dias */
...
export async function registrarOPUBatch(formData: FormData): Promise<{ error: string | null }> {
  ...
  // Tipo interno: PROPRIA → "REALIZADA", ADQUIRIDA → "ADQUIRIDA" (≠ COMPRADA que é prenhez comprada)
  const tipoInterno = tipo_sessao === "ADQUIRIDA" ? "ADQUIRIDA" : "REALIZADA";
