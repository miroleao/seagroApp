...
function BigFinancialCard({ label, valor, sub, icon: Icon, color, href }: {
  label: string; valor: string; sub?: string;
  icon: React.ElementType; color: string; href?: string;
}) {
  const inner = (
    <div className="card p-6 flex items-center gap-5 hover:shadow-md transition-shadow h-full">
      <div className={`p-4 rounded-2xl ${color} shrink-0`}>
        <Icon className="w-7 h-7 text-white" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm text-gray-500">{label}</p>
        <p className="text-3xl font-bold text-gray-900 mt-0.5 tracking-tight leading-none">{valor}</p>
...
export default async function DashboardPage() {
  ...
  // ── Novos cards: Prenhas, Efetivados, Patrimônio, Vendas ───────────────────
  const [
    { count: totalPrenhas },
    { count: totalEfetivados },
    { data: doadorasValor },
    { data: vendasData },
  ] = await Promise.all([
    // Receptoras prenhas (PRENHA ou PRENHA_EMBRIAO)
    supabase.from("animals").select("*", { count: "exact", head: true })
      .eq("farm_id", FARM_ID)
      .in("status_rebanho", ["PRENHA", "PRENHA_EMBRIAO"]),

    // Embriões efetivados (implantados em receptoras)
    supabase.from("embryos").select("*", { count: "exact", head: true })
      .eq("farm_id", FARM_ID)
      .eq("status", "IMPLANTADO"),

    // Doadoras com valor de parcela para calcular patrimônio
    supabase.from("animals").select("valor_parcela, percentual_proprio")
      .eq("farm_id", FARM_ID)
      .eq("tipo", "DOADORA")
      .not("valor_parcela", "is", null),

    // Vendas realizadas
    supabase.from("transactions").select("id, valor_total, tipo")
      .eq("farm_id", FARM_ID)
      .eq("tipo", "VENDA"),
  ]);

  // Patrimônio estimado: soma de (parcela / % próprio) × 30 por doadora
  const patrimonioTotal = (doadorasValor ?? []).reduce((acc: number, d: any) => {
    if (!d.valor_parcela) return acc;
    const perc = d.percentual_proprio && d.percentual_proprio > 0
      ? d.percentual_proprio
      : 1;
    return acc + (d.valor_parcela / perc) * 30;
  }, 0);

  // Total de vendas
  const vendasTotal = (vendasData ?? []).reduce((acc: number, v: any) => {
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      {/* Cards — Linha 2: Financeiro (destaque) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <BigFinancialCard
          label="Patrimônio Estimado"
          valor={patrimonioTotal > 0 ? formatCurrency(patrimonioTotal) : "—"}
          sub="valorização das doadoras × 30 (parcela ÷ % próprio)"
          icon={Wallet}
        ...
        <BigFinancialCard
          ...
          valor={vendasTotal > 0 ? formatCurrency(vendasTotal) : "R$ 0,00"}
          sub={`${(vendasData ?? []).length} venda(s) registrada(s) no financeiro`}
          icon={Tag}
          color="bg-blue-600"
          href="/financeiro"
        />
