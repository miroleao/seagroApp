...
export async function salvarInfoLeilao(formData: FormData) {
  ...
  if (selectErr) {
    console.error("[salvarInfoLeilao] SELECT error:", selectErr);
  ...
  if (existing?.id) {
    const { error: updateErr } = await supabase
      ...
      .eq("id", existing.id);
    if (updateErr) {
      console.error("[salvarInfoLeilao] UPDATE error:", updateErr);
    ...
    if (insertErr) {
      console.error("[salvarInfoLeilao] INSERT error:", insertErr);
