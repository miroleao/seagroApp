...
import { vincularCriaManual } from "./actions";
...
export function VincularCriaManual({ animalId, animaisDisponiveis }: Props) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    fd.append("cria_id",   selecionado.id);
    startTransition(async () => {
      const res = await vincularCriaManual(fd);
      if (res.ok) { router.refresh(); setAberto(false); }
      else setErro(res.erro ?? "Erro ao vincular");
    });
  }
